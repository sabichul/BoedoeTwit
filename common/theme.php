...
function theme_page($title, $content) {
	...
	if (DEBUG_MODE == 'ON') {
		...
		$body .= '<p>Processed in '.round($time, 4).' seconds ('.round(($time - $api_time - $services_time) / $time * 100).'% '.CLIENT_NAME.', '.round($api_time / $time * 100).'% Twitter, '.round($services_time / $time * 100).'% other services). '.$rate_limit.'.</p>';
	}
    $meta = '';
	if ($title == 'Login') {
		$title = ''.CLIENT_NAME.' - mobile Twitter Login';
	...
	{
		//	If the cookies haven't been set, remind the user that they can set how looks
		/*echo			'<p>Think '.CLIENT_NAME.' looks ugly? <a href="settings">Change the colours!</a></p>';*/
	}
	echo '		</body>
				</html>';
	exit();
...
function theme_css() {
	...
	.avatar{display:block; height:48px; width:48px; left:0.3em; margin:0; overflow:hidden; position:absolute;}
	.status{display:block;word-wrap:break-word;}
	.shift{margin-left:55px;min-height:24px;}
	.from{font-size:0.75em;color:#{$c->small};font-family:serif;}
	.from a{color:#{$c->small};}
	.kanan{float:right;padding:5px;font-size:0.8em;font-weight:bold;color:#{$c->small}}
</style>";
}
