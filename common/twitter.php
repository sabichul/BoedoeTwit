...
function twitter_confirmed_page($query)
{
	...
	switch ($action) {
                case 'block':
			$content  = "<p><span class='avatar'><img src='".CLIENT_LOGO."' width='48' height='48' /></span><span class='status shift'>Bye-bye @$target - you are now <strong>blocked</strong>.</span></p>";
                        break;
                case 'unblock':
                        $content  = "<p><span class='avatar'><img src='".CLIENT_LOGO."' width='48' height='48' /></span><span class='status shift'>Hello again @$target - you have been <strong>unblocked</strong>.</span></p>";
                        break;
                case 'spam':
                        $content = "<p><span class='avatar'><img src='".CLIENT_LOGO."' width='48' height='48' /></span><span class='status shift'>Yum! Yum! Yum! Delicious spam! Goodbye @$target.</span></p>";
                        break;
	}
...
function theme_directs_menu() {
	return '<p class="date"><a href="directs/create">Create</a> | <a href="directs/inbox">Inbox</a> | <a href="directs/sent">Sent</a></p>';
}
...
function theme_timeline($feed, $paginate = true) {
	...
	foreach ($feed as $status) {
		...
		$html = "<b><a href='user/{$status->from->screen_name}'>{$status->from->screen_name}</a></b> <span class='kanan'>$link{$retweeted}</span><br />$actions<br />{$text}<br />$media<span class='from'>$source</span>";
...
function theme_pagination($max_id = false) {
	...
	if($max_id) {
		$links[] = "<a href='{$_GET['q']}?max_id=".$max_id."$query' accesskey='9'>Older</a>";
	}
	else {
		if ($page == 0) $page = 1;
		$links[] = "<a href='{$_GET['q']}?page=".($page+1)."$query' accesskey='9'>Older</a>";
		if ($page > 1) $links[] = "<a href='{$_GET['q']}?page=".($page-1)."$query' accesskey='8'>Newer</a>";
	}
	if($query) {
		$query = '?' . substr($query, 1);
	}
	$links[] = "<a href='{$_GET['q']}?$query'>First</a>";
	return '<p class="date">'.implode(' | ', $links).'</p>';
}
